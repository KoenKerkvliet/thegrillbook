...
<title>Er is een video met je gedeeld</title>
</head>
<body style="margin:0;padding:0;background:#F2EDE4;font-family:Arial,Helvetica,sans-serif;">
  <div style="display:none;max-height:0;overflow:hidden;font-size:1px;line-height:1px;color:#fff;opacity:0;">
    ${sharerName} deelde een video met je.
  </div>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background:#ffffff;max-width:600px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;color:#1a1a1a;font-size:15px;line-height:1.6;">
              <h1 style="margin:0 0 16px;font-size:20px;color:#0B0B0B;">Er is een video met je gedeeld</h1>
              <p style="margin:0 0 24px;"><strong>${sharerName}</strong> (@${sharerUsername}) deelde een video met je.</p>
...
${sharerName} (@${sharerUsername}) deelde een video met je.
...
serve(async (req) => {
  ...
  try {
    ...
    const { videoId, sharedWithUserId } = await req.json()
    if (!videoId || !sharedWithUserId) {
      return json({ success: false, error: 'videoId en sharedWithUserId zijn verplicht.' }, 400)
    }
    ...
    // endpoint being used to email arbitrary people about arbitrary videos.
    ...
    await sendEmail({
      to: recipientEmail,
      subject: 'Er is een video met je gedeeld',
