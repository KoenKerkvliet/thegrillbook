...
<title>Iemand heeft je BBQ-moment geliket</title>
</head>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="background:#ffffff;max-width:600px;width:100%;">
          ...
          <tr>
            <td style="padding:32px;color:#1a1a1a;font-size:15px;line-height:1.6;">
              <h1 style="margin:0 0 16px;font-size:20px;color:#0B0B0B;">Iemand heeft je BBQ-moment geliket</h1>
              <p style="margin:0 0 24px;"><strong>${likerName}</strong> (@${likerUsername}) liket je BBQ-moment.</p>
...
${likerName} (@${likerUsername}) liket je BBQ-moment.
...
serve(async (req) => {
  ...
  try {
    ...
    if (!momentId) return json({ success: false, error: 'momentId is verplicht.' }, 400)
    ...
    const [{ data: moment }, { data: likerProfile }] = await Promise.all([
    ...
    if (!moment || !likerProfile || moment.owner_id === liker.id) {
      return json({ success: true }) // nothing to notify
    }

    const { data: owner } = await admin.auth.admin.getUserById(moment.owner_id)
    ...
    await sendEmail({
      to: ownerEmail,
      subject: 'Iemand heeft je BBQ-moment geliket',
